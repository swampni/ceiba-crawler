...
class Crawler():
	def __init__(self, user_name, password):
		...
		for link in self.links:
			...
			self.browser.get("https://ceiba.ntu.edu.tw/modules/index.php?csn="+str(_info[0])+"&default_fun=info&current_lang=chinese")
			self.browser.switch_to.frame('mainFrame')
			course_name = self.browser.find_element_by_xpath('//body/div/div/div/div/table/tbody/tr/td').text
			course_time = self.browser.find_element_by_xpath('//body/div/div/div/div/table/tbody/tr[6]/td').text
	...
	def get_homework(self, course):
		self.browser.get("https://ceiba.ntu.edu.tw/modules/index.php?csn="+str(course[0])+"&default_fun=hw&current_lang=chinese")
		try:
			self.browser.switch_to.frame('mainFrame')
			entites = len(self.browser.find_elements_by_xpath('//body/div/div/div/div/table/tbody/tr'))
			for i in range(1,entites):
				self.browser.find_element_by_xpath('//body/div/div/div/div/table/tbody/tr['+str(i+1)+']/td/a').click()
	...
	def get_bulletin(self, course):
		self.browser.get("https://ceiba.ntu.edu.tw/modules/index.php?csn="+str(course[0])+"&default_fun=bulletin&current_lang=chinese")
		try:
			self.browser.switch_to.frame('mainFrame')
			entites = len(self.browser.find_elements_by_xpath('//body/div/div/div/div/table/tbody/tr')[1:])
			for i in range(1,entites):
				self.browser.find_element_by_xpath('//body/div/div/div/div/table/tbody/tr['+str(i+1)+']/td/a').click()
				self.browser.switch_to.default_content()
				self.browser.switch_to.frame('mainFrame')
